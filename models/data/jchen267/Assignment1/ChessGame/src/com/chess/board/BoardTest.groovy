...
class BoardTest extends GroovyTestCase {
    ...
    void testIsCoordinateValid() {
        ...
        for (int i=0; i<testsCt; ++i) {
            int randomX = ThreadLocalRandom.current().nextInt(0, this.defaultSize);
            int randomY = ThreadLocalRandom.current().nextInt(0, this.defaultSize);
            assertEquals(true, board.isCoordinateValid(randomX, randomY));
        }

        for (int i=0; i<testsCt; ++i) {
            int randomX = ThreadLocalRandom.current().nextInt(-100, 0);
            int randomY = ThreadLocalRandom.current().nextInt(0, this.defaultSize);
            assertEquals(false, board.isCoordinateValid(randomX, randomY));
            assertEquals(false, board.isCoordinateValid(randomY, randomX));
        }
