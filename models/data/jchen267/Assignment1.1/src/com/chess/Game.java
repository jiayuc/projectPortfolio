...
public class Game {
     ...
     * @param nextMoveMakter user that should make the next move
     */
    private void runGame(User nextMoveMakter) {
        while (!this.board.isGameOver(nextMoveMakter)) {
            this.board.printBoard();
            System.out.println("Next!" + nextMoveMakter.getName() + "'s turn...");

            // 1) locate piece to move
            Piece pieceToMove = getPieceToMove(nextMoveMakter);
            System.out.println("---" + nextMoveMakter + "has chosen to move " + pieceToMove.getPieceType() + ". Legal moves are: ");
            ...
            System.out.println("===" + nextMoveMakter + "moved. ");
            nextMoveMakter = nextMoveMakter == users[0] ? users[1] : users[0];
        }
    }
    ...
    private Piece getPieceToMove(User nextMoveMaker) {
        ...
        while (pieceToMove == null || pieceToMove.getOwner() != nextMoveMaker ||
                pieceToMove.getLegalMoves(this.board).isEmpty()) {
