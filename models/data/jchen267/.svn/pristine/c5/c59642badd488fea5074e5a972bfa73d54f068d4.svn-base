    ...
    void testUpdateLegalMoves() {
        for (int i=0; i<this.pawns.length; ++i) {
            for (int j=0; j<this.pawns[i].size(); ++j) {
                ...
                assertEquals(2, pawn.getLegalMoves(this.board).size())
        ...
        for (int i=0; i<this.pawns.length; ++i) {
            for (int j=0; j<this.pawns[i].size(); ++j) {
                ...
                for (int step=0; step<2; ++step) {
                    assertEquals(1, pawn.getLegalMoves(this.board).size())
                    oldX = pawn.getCoordinate().getx()
                    assertEquals(0, pawn.move(this.board, oldX + moveXDir*1, oldY))
                }
                int expectedMoveCt = (oldY == 0 || oldY == this.defaultSize-1) ? 2 : 3
                // TMP
//                this.board.printBoard();
//                pawn.printLegalMoves();
                assertEquals(expectedMoveCt, pawn.getLegalMoves(this.board).size())
                pawn.restoreToInitState()
                this.board.movePiece(pawn, oldX_cp, oldY_cp)
            }
        }
    }
